...
Zeroing $\mathbf{x}_{\text{clin}} = \mathbf{0}$ alone does not reliably drive the attention MLP to $\beta \approx 0$.
...
\textbf{Report Drafting} & Advancing (supervised) & Ambient scribes and RL report frameworks (e.g., UniRG) target clinical hallucination reduction. Note: $>$1{,}000 FDA AI authorizations are mostly imaging \emph{devices}, not generative report writers. \\[1ex]
...
\textbf{Diagnostic Support} & Research / supervised use & Strong results on selected benchmarks (e.g., NEJM Image Challenge MCQs); not equivalent to unsupervised clinical diagnosis. Chain-of-thought agents are an active research direction. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{description}[style=nextline, leftmargin=1em, labelindent=0pt, itemsep=0.2ex]
\item[Retrieval-Augmented Generation (RAG)] Ground LLM responses in retrieved documents from verified sources (clinical guidelines, peer-reviewed literature), reducing reliance on parametric memory that may contain errors. We demonstrate the RAG mechanism in \notebook{03\_llm\_assisted\_computing.ipynb} with ChromaDB and sentence-transformer embeddings over a small teaching corpus; production deployments should index verified guidelines and peer-reviewed literature.
