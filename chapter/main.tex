...
The BraTS dataset ($\approx$3 GB for 100 cases) is hosted on GitHub Releases and automatically downloaded at runtime.
...
We demonstrate both approaches in \notebook{01\_medical\_imaging.ipynb}.
...
In \notebook{02\_multimodal\_integration.ipynb}, we demonstrate that GCNs maintain reasonable accuracy even when only 50\% of patients have labeled outcomes, effectively propagating labels through the similarity structure.
...
Our adaptive fusion architecture (Section~\ref{sec:multimodal}) naturally handles masking.
Consider a patient with imaging data but missing clinical records.
We set $\mathbf{x}_{\text{clin}} = \mathbf{0}$, yielding an uninformative encoding $\mathbf{z}_{\text{clin}} = f_{\text{clin}}(\mathbf{0})$.
The weighting network $g$, trained on complete cases with modality dropout, learns to recognize this pattern and assigns $\beta \approx 0$, shifting weight to imaging: $\mathbf{z}_{\text{fused}} \approx \alpha \cdot \mathbf{z}_{\text{img}}$ with $\alpha \approx 1$.
...
We demonstrated promptable segmentation in \notebook{01\_medical\_imaging.ipynb}, where a single bounding box prompt yields segmentation propagated across entire volumes.
